...
@app.route('/admin/ml/debug_matching')
@login_required
def debug_ml_matching():
    """Debug why predictions aren't matching actual games"""
    try:
        html = "<h2>ML Prediction Matching Debug</h2>"
        
        # Get a few pending predictions
        pending = CFBPredictionLog.query.filter(
            CFBPredictionLog.week == '1',
            CFBPredictionLog.game_completed == False
        ).limit(5).all()
        
        html += f"<h3>Sample Pending Predictions ({len(pending)})</h3>"
        
        for pred in pending:
            html += f"""
            <div style='border: 1px solid #ccc; margin: 10px; padding: 10px;'>
                <strong>Prediction:</strong> {pred.home_team} vs {pred.away_team} (Winner: {pred.predicted_winner})<br>
            """
            
            # Try to find matching game
            exact_match = Game.query.filter(
                Game.week == '1',
                Game.home_team == pred.home_team,
                Game.away_team == pred.away_team
            ).first()
            
            reverse_match = Game.query.filter(
                Game.week == '1',
                Game.home_team == pred.away_team,
                Game.away_team == pred.home_team
            ).first()
            
            if exact_match:
                html += f"<span style='color: green;'>✓ EXACT MATCH FOUND:</span> {exact_match.home_team} {exact_match.home_score}-{exact_match.away_score} {exact_match.away_team}<br>"
            elif reverse_match:
                html += f"<span style='color: orange;'>⚠ REVERSE MATCH FOUND:</span> {reverse_match.home_team} {reverse_match.home_score}-{reverse_match.away_score} {reverse_match.away_team}<br>"
            else:
                html += "<span style='color: red;'>✗ NO MATCH FOUND</span><br>"
                
                # Show what games DO exist for these teams
                team1_games = Game.query.filter(
                    Game.week == '1'
                ).filter(
                    db.or_(Game.home_team == pred.home_team, Game.away_team == pred.home_team)
                ).all()
                
                team2_games = Game.query.filter(
                    Game.week == '1'
                ).filter(
                    db.or_(Game.home_team == pred.away_team, Game.away_team == pred.away_team)
                ).all()
                
                if team1_games:
                    html += f"Games involving {pred.home_team}: "
                    for g in team1_games:
                        html += f"{g.home_team} vs {g.away_team}; "
                    html += "<br>"
                
                if team2_games:
                    html += f"Games involving {pred.away_team}: "
        ...
        html += "<br><a href='/admin/ml'>Back to ML Dashboard</a>"
        return html
...
@app.route('/admin/ml/update_existing_games')
@login_required
def update_existing_week1_games():
    """Manually trigger ML logging for existing Week 1 games"""
    if not CFB_ML_ENABLED:
        return "ML tracking not enabled"
    ...
    try:
        ...
        for game in week1_games:
            # Call the ML logging function for this game
            updated_count = log_game_result_to_ml(
                game.home_team, 
                game.away_team, 
                game.home_score, 
                game.away_score, 
            ...
            results.append(f"{game.home_team} vs {game.away_team}: {updated_count} predictions updated")
        
        return f"""
        <h2>ML Update Results</h2>
        <p><strong>Total predictions updated:</strong> {total_updated}</p>
        <h3>Details:</h3>
        <ul>
        {"".join(f"<li>{result}</li>" for result in results)}
        </ul>
...
@app.route('/admin/ml/show_pending')
@login_required
def show_pending_predictions():
    """Show which predictions are still pending and why"""
    try:
        pending = CFBPredictionLog.query.filter(
            CFBPredictionLog.week == '1',
            CFBPredictionLog.game_completed == False
        ...
        html = f"<h2>Still Pending: {len(pending)} predictions</h2>"
        
        for pred in pending:
            html += f"""
            <div style='border: 1px solid #orange; margin: 5px; padding: 10px;'>
                <strong>Pending:</strong> {pred.home_team} vs {pred.away_team} (Predicted: {pred.predicted_winner})<br>
            """
            
            # Check if there's a corresponding game
            game = Game.query.filter(
                Game.week == '1'
            ).filter(
                db.or_(
                    db.and_(Game.home_team == pred.home_team, Game.away_team == pred.away_team),
                    db.and_(Game.home_team == pred.away_team, Game.away_team == pred.home_team)
                )
            ).first()
            
            if game:
                winner = game.home_team if game.home_score > game.away_score else game.away_team
                html += f"<span style='color: red;'>Game exists but not matched:</span> {game.home_team} {game.home_score}-{game.away_score} {game.away_team} (Winner: {winner})<br>"
            else:
                html += "<span style='color: red;'>No corresponding game found</span><br>"
...
@app.route('/admin/ml/check_neutral_games')
@login_required
def check_neutral_games():
    """Check neutral site status of problematic games"""
    try:
        problem_games = [
            ('Nebraska', 'Cincinnati'),
            ('Cincinnati', 'Nebraska'),
        ...
        html = "<h2>Neutral Site Game Check</h2>"
        
        for home, away in problem_games:
            game = Game.query.filter(
                Game.week == '1',
                Game.home_team == home,
                Game.away_team == away
            ...
            if game:
                neutral_status = "YES" if game.is_neutral_site else "NO"
                html += f"<p>{game.home_team} vs {game.away_team}: Neutral = {neutral_status}</p>"
...
@app.route('/admin/ml/debug_flipped_matching')
@login_required
def debug_flipped_matching():
    """Debug why flipped matching isn't working"""
    try:
        html = "<h2>Debug Flipped Matching</h2>"
        
        # Get the specific pending predictions
        pending = CFBPredictionLog.query.filter(
            CFBPredictionLog.week == '1',
            CFBPredictionLog.game_completed == False,
            CFBPredictionLog.home_team != 'FCS'  # Skip FCS vs FCS
        ).all()
        
        for pred in pending:
            html += f"""
            <div style='border: 1px solid #ccc; margin: 10px; padding: 10px;'>
                <strong>Prediction:</strong> {pred.home_team} (home) vs {pred.away_team} (away)<br>
            """
            
            # Look for exact match
            exact_game = Game.query.filter(
                Game.week == '1',
                Game.home_team == pred.home_team,
                Game.away_team == pred.away_team
            ).first()
            
            # Look for flipped match
            flipped_game = Game.query.filter(
                Game.week == '1',
                Game.home_team == pred.away_team,
                Game.away_team == pred.home_team
            ).first()
            ...
            if exact_game:
                html += f"<span style='color: green;'>EXACT MATCH:</span> {exact_game.home_team} {exact_game.home_score}-{exact_game.away_score} {exact_game.away_team}<br>"
            elif flipped_game:
                html += f"<span style='color: orange;'>FLIPPED MATCH:</span> {flipped_game.home_team} {flipped_game.home_score}-{flipped_game.away_score} {flipped_game.away_team}<br>"
                html += f"<span style='color: blue;'>Should update this prediction</span><br>"
            else:
                html += "<span style='color: red;'>NO MATCH FOUND</span><br>"
...
@app.route('/admin/ml/fix_these_4_predictions')
@login_required
def fix_these_4_predictions():
    """Manually fix the 4 specific flipped predictions"""
    try:
        updated = 0
        
        # Define the 4 problematic prediction-game pairs
        fixes = [
            # (prediction_home, prediction_away, actual_game_home, actual_game_away)
            ('Nebraska', 'Cincinnati', 'Cincinnati', 'Nebraska'),
            ('Appalachian St', 'Charlotte', 'Charlotte', 'Appalachian St'), 
            ('Syracuse', 'Tennessee', 'Tennessee', 'Syracuse'),
            ('Virginia Tech', 'South Carolina', 'South Carolina', 'Virginia Tech')
        ]
        
        for pred_home, pred_away, game_home, game_away in fixes:
            # Find the prediction
            prediction = CFBPredictionLog.query.filter(
                CFBPredictionLog.week == '1',
                CFBPredictionLog.home_team == pred_home,
                CFBPredictionLog.away_team == pred_away,
                CFBPredictionLog.game_completed == False
            ).first()
            ...
            game = Game.query.filter(
                Game.week == '1',
                Game.home_team == game_home,
                Game.away_team == game_away
            ).first()
            
            if prediction and game:
                ...
                if prediction.predicted_margin:
                    prediction.margin_error = abs(prediction.predicted_margin - actual_margin)
                
                updated += 1
        
        db.session.commit()
        return f"Successfully updated {updated} predictions"
...
@app.route('/admin/ml/debug_prediction_types')
@login_required  
def debug_prediction_types():
    """Debug the type error in prediction logging"""
    try:
        # Test with a real scheduled game
        test_game = ScheduledGame.query.filter_by(week='2', completed=False).first()
        if not test_game:
            return "No Week 2 scheduled games found to test"
        
        home_team = test_game.home_team
        away_team = test_game.away_team
        
        html = f"<h2>Debug Prediction Types: {away_team} @ {home_team}</h2>"
        
        # Step 1: Test the prediction function
        html += "<h3>Step 1: Testing prediction function</h3>"
        prediction_result = predict_matchup_ultra_enhanced(home_team, away_team, 'neutral')
        
        for key, value in prediction_result.items():
            html += f"<p>{key}: {value} (Type: {type(value)})</p>"
        
        # Step 2: Test the total calculation
        html += "<h3>Step 2: Testing total calculation</h3>"
        home_stats = TeamStats.query.filter_by(team_name=home_team).first()
        away_stats = TeamStats.query.filter_by(team_name=away_team).first()
        ...
        if home_stats and away_stats:
            ...
            html += f"<p>home_avg: {home_avg} (Type: {type(home_avg)})</p>"
            html += f"<p>away_avg: {away_avg} (Type: {type(away_avg)})</p>"
            
            try:
                predicted_total = round(home_avg + away_avg, 1)
                html += f"<p>predicted_total: {predicted_total} (Type: {type(predicted_total)})</p>"
            except Exception as e:
                html += f"<p style='color: red;'>Error calculating total: {e}</p>"
        
        # Step 3: Test critical field assignments
        html += "<h3>Step 3: Testing critical assignments</h3>"
        try:
            final_margin = prediction_result['final_margin']
            html += f"<p>final_margin: {final_margin} (Type: {type(final_margin)})</p>"
            
            abs_margin = abs(final_margin)
            html += f"<p>abs(final_margin): {abs_margin} (Type: {type(abs_margin)})</p>"
        except Exception as e:
            html += f"<p style='color: red;'>Error with margin: {e}</p>"
        
        try:
            win_prob = prediction_result['win_probability']
            html += f"<p>win_probability: {win_prob} (Type: {type(win_prob)})</p>"
        except Exception as e:
            html += f"<p style='color: red;'>Error with win_probability: {e}</p>"
        
        return html
        
    except Exception as e:
        return f"<h2>Debug Error:</h2><p>{str(e)}</p><p>Exception type: {type(e)}</p>"


@app.route('/admin/ml/test_prediction_creation')
@login_required
def test_prediction_creation():
    """Test actually creating a prediction to catch the type error"""
    try:
        # Get a test game
        test_game = ScheduledGame.query.filter_by(week='2', completed=False).first()
        if not test_game:
            return "No Week 2 games to test"
        
        html = f"<h2>Testing Prediction Creation: {test_game.away_team} @ {test_game.home_team}</h2>"
        
        # Test the auto_predict_single_game function directly
        success, message, pred_id = auto_predict_single_game(test_game, overwrite=True)
        
        if success:
            html += f"<p style='color: green;'>SUCCESS: {message}</p>"
            html += f"<p>Prediction ID: {pred_id}</p>"
        else:
            html += f"<p style='color: red;'>FAILED: {message}</p>"
        ...
        import traceback
        return f"""
        <h2>Prediction Creation Error</h2>
...
@app.route('/admin/ml/test_batch_prediction')
@login_required
def test_batch_prediction():
    """Test batch predicting to find the type error"""
    try:
        # Get first 3 Week 2 games to test
        test_games = ScheduledGame.query.filter_by(week='2', completed=False).limit(3).all()
        
        html = f"<h2>Testing Batch Prediction (3 games)</h2>"
        ...
        for i, game in enumerate(test_games):
            try:
                html += f"<h3>Game {i+1}: {game.away_team} @ {game.home_team}</h3>"
                success, message, pred_id = auto_predict_single_game(game, overwrite=True)
                ...
                html += f"<p style='color: red;'>ERROR on game {i+1}: {str(e)}</p>"
                failed += 1
        
        html += f"<h3>Results: {successful} successful, {failed} failed</h3>"
        return html
        
    except Exception as e:
        import traceback
        return f"""
        <h2>Batch Test Error</h2>
        <p>{str(e)}</p>
        <pre>{traceback.format_exc()}</pre>
        """


@app.route('/admin/ml/debug_predict_week/<int:week>')
@login_required
def debug_predict_week(week):
    """Debug version of auto_predict_week to catch the type error"""
    try:
        html = f"<h2>Debug Auto-Predict Week {week}</h2>"
        ...
        html += f"<p>Found {len(unpredicted_games)} unpredicted games</p>"
        
        successful_predictions = 0
        failed_predictions = 0
        skipped_predictions = 0
        
        for i, game in enumerate(unpredicted_games):
            try:
                html += f"<h3>Processing game {i+1}: {game.away_team} @ {game.home_team}</h3>"
                ...
                if existing:
                    skipped_predictions += 1
                    html += "<p>⭐ Already predicted - skipping</p>"
                    continue
                ...
                html += f"<p style='color: red;'>ERROR on game {i+1}: {str(game_error)}</p>"
                html += f"<p>Game data: home={game.home_team}, away={game.away_team}, week={game.week}</p>"
                failed_predictions += 1
        
        # Test the summary calculation that might be causing the error
        try:
            summary = f'Week {week} Auto-Prediction Results: {successful_predictions} successful, {failed_predictions} failed, {skipped_predictions} skipped'
            html += f"<h3>Summary: {summary}</h3>"
        except Exception as summary_error:
            html += f"<p style='color: red;'>ERROR in summary: {str(summary_error)}</p>"
